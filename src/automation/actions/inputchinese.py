...
class InputChineseItem(SimpleCardWidget):

    class InputChineseAttributeItem(SimpleCardWidget):
        ...
        def init_data(self):
            try:
                #使用父类的对应值
                self.chinese.setText(self.parent_mouse_widget_item.chinese.text())
        ...
        def confirm_click(self):
            try:
                # 获取输入的坐标值
                chinese = self.chinese.text()
                # 获取父类的对应值
                self.parent_mouse_widget_item.chinese.setText(chinese)
    ...
    def __init__(self, icon_path=None):
        ...
        # 左侧：拾取坐标按钮
        pick_button = PrimaryToolButton(Icon('CN').icon)
        ...
        main_layout.addWidget(self.chinese)
        #监听数值改变
    ...
    def get_attribute_widget_class(self):
        self.atribute_widget = self.InputChineseAttributeItem(self)
    ...
    def init_attribute_dict(self,index):
        try:
            value =  {
                "index":index,
                "name":"输入中文",
                "type":"InputChineseItem",
                "properties":{
                    "chinese": ""
                },
                "method": "input_chinese"
            }
            print("索引信息", index)
            print(value)
            self.index = index
    ...
    def initialisation(self):
        ...
        for i in range(len(data["actions"])):
            if data["actions"][i]["index"] == self.index:
                self.chinese.setText(data["actions"][i]["properties"]["chinese"])
                print("英文输入控件数据初始化成功")
    #记录索引信息
    ...
    def listeningChanged(self):
        self.chinese.textChanged.connect(self.valueChanged)
    ...
    def valueChanged(self):
        ...
        try:
            ...
            for i in range(len(data["actions"])):
                if data["actions"][i]["index"] == self.index:
                    data["actions"][i]["properties"]["chinese"] = chinese
